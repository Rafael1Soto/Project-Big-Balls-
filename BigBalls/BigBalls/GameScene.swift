...
//  BigBalls
//
//  Created by A.M. Student on 4/12/21.
//
...
class GameScene: SKScene {
    ...
    var score = 0 {
        didSet {
            ...
            let formattedScore = formatter.string(from: score as NSNumber)
                ?? "0"
    ...
    override func didMove(to view: SKView) {
        ...
        for j in stride(from: 100, to: view.bounds.width - marbleRadius, by: marble.frame.height) {
...
//    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if matchedMarbles.count >= 3 {
            score += Int(pow(2, Double(min(matchedMarbles.count, 1))))
